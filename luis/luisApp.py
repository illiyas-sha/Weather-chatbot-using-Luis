...
class LuisConnect(ActivityHandler):
    def __Init(self):
        ...
        self.configuration = self.config_reader.read_cofig()
    ...
    async def on_message_activity(self,turn_contect:TurnContext):
        weather_info = WeatherInformation()
        luis_results = await self.luis_recognizer.recognize(turn_context)
        result = luis_results.properties["luisResult"]
